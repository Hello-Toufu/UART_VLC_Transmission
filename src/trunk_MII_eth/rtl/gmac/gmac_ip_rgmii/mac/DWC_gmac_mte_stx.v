...
// Internal reg and wire declarations.
  reg  [3:0]   ifg_counter;           // Inter frame Gap(period) counter.
  reg  [1:0]   slot_delay;            // Mod-4 counter used in GMII mode.
  wire [3:0]   ifg_value;             // actual Inter frame Gap.
...
// Parameter declarations
  parameter IFG_LMT_GMII = 4'b1100;
...
//---------------
  ...
  assign ifg_value = IFG_LMT_GMII - {1'b0,ifg_min}; 
  assign ifg_clr = ~ifg_counter[3]; 
  assign ifg_cnt_lmt = (ifg_counter == IFG_LMT_GMII); 
  assign ifg_ok = (ifg_counter >= (ifg_value - 4'b0010));
...
//------------
  ...
  assign pause_quanta_tic = (&slot_timer[3:0]) & (&slot_delay);

...
// Input and Output port declarations.
  ...
  // MMC Status Inputs
  input           txstatus_valid;       // status valid for the transmit status
  input           frx2mmc_ctrlframe;    // pause control frame received 


  // MMC Status Outputs
  output          sca_txstatus_valid;   // transmit status valid sync to App clk
                                            // sync'd to Application clock.
  output          sca_pauseframe;       // pause frame rcvd sync to App clk
  output          sca_jumboframe_en;    // Enable for jumbo frames
...
// Internal Reg and wire declarations
  ...
  reg               scr_portselect;   // Portselect bit sync'd to GMAC Rx clock.
  reg               sct_portselect;   // Port select bit sync'd to GMAC Tx clock.
  ...
  wire              txstatus_valid_sync;     // txstatus_valid sync'd to clk_csr_i;
  wire              pauseframe_sync;         // pauseframe sync'd to clk_csr_i;
...
// config register write pulse synchroniser(Tx clock).
  ...
  assign sca_txstatus_valid = txstatus_valid_sync;
  assign sca_pauseframe = pauseframe_sync;
...
// Detect rising edge of ptp_aux_ts_trig_i_sync to create a trig. pulse 
    ...
    //----------------------------------------
    // synchronizing with respect to clk_csr_i
    //----------------------------------------
   
        DWC_gmac_bcm22
         #(0,2,0,2,0) txsts_vld_sync_app_clk (
                                       .clk_s       (clk_tx_i), 
                                       .rst_s_n     (rst_clk_tx_async_n),
                                       .init_s_n    (rst_clk_tx_sync_n),
                                       .event_s     (txstatus_valid), 
                                       .clk_d       (clk_csr_i), 
                                       .rst_d_n     (rst_clk_csr_async_n), 
                                       .init_d_n    (rst_clk_csr_sync_n), 
                                       .event_d     (txstatus_valid_sync),
                                       .test        (1'b0)
                                      );
        ...
        DWC_gmac_bcm22
         #(0,2,0,2,0) ctrlframe_sync_app_clk (
                                       .clk_s       (clk_rx_i), 
                                       .rst_s_n     (rst_clk_rx_async_n), 
                                       .init_s_n    (rst_clk_rx_sync_n), 
                                       .event_s     (frx2mmc_ctrlframe), 
                                       .clk_d       (clk_csr_i),
                                       .rst_d_n     (rst_clk_csr_async_n), 
                                       .init_d_n    (rst_clk_csr_sync_n), 
                                       .event_d     (pauseframe_sync),
                                       .test        (1'b0)
                                      );
       ...
       assign sca_jumboframe_en = cr_reg0[20];

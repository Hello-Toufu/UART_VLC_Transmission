...
// Input and Output Port declarations.
  ...
  // MMC data and interrupt
  input [31:0]     mmc_rdata;              // Read data from MMC registers.
  input            mmc_intr;               // Interrupt from MMC block
  input            mmc_rx_intr;            // Interrupt from MMC block (Rx Registers).
  input            mmc_tx_intr;            // Interrupt from MMC block (Tx Registers).
  input            mmc_ipc_rx_intr;        // Interrupt from MMC block (IPC Rx Registers).
  input            mmc_ipc_tx_intr;        // Interrupt from MMC block (IPC Tx Registers).
...
// wire declarations for outputs
        ...
        if (~rst_clk_csr_n) begin
          ...
          if (mci_val_i) begin
            if (mac_csr_sel & mci_rdwrn_i) 
              mci_rdata_o <= rdata;
            else if (mmc_csr_sel & mci_rdwrn_i) 
              mci_rdata_o <= mmc_rdata;
  ...
  // MAC Configuration register
   ...
   assign port_select = config_reg[15];
  ...
  always @(`CSR_RST_MODE) begin
    if (~rst_clk_csr_n) begin
      ...
      if (reg_wr & mci_ack & reg_sel[0]) begin
        ...
        config_reg[15]  <= (mci_be_i[1]) ? mci_wdata_i[15] : config_reg[15];
        config_reg[24] <= `L;
        config_reg[14] <= `L;
        config_reg[8]  <= `L;
...
// MAC frame filter register
  ...
  assign filter_reg_rdata = {filter_reg[31],{21{`L}},filter_reg[9:3],2'b00,filter_reg[0]};
  ...
  always @(`CSR_RST_MODE) begin
    if (~rst_clk_csr_n) begin
      ...
      if (reg_wr & mci_ack & reg_sel[1]) begin
        ...
        filter_reg[10]  <= `L;
        filter_reg[9:8] <= (mci_be_i[1]) ? mci_wdata_i[9:8] : filter_reg[9:8];
        filter_reg[7:3] <= (mci_be_i[0]) ? mci_wdata_i[7:3] : filter_reg[7:3];
        filter_reg[2:1] <= {2{`L}};
        filter_reg[0]   <= (mci_be_i[0]) ? mci_wdata_i[0] : filter_reg[0];
      end
    end
  end  

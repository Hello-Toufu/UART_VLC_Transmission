...
// Internal reg and wire declarations
  ...
  reg              nibble_sel;          // Nibble select qualifying the upper/lower 
                                          // nibble(MII).
...
// Upper/lower Nibble select for MII
    always @(`TX_RST_MODE) begin
      if (~rst_clk_tx_n) begin
        ...
        if (~sct_portselect) begin
          nibble_sel <= `L;
        end
        else if (tpe_state[TPE_IDLE]) begin
          nibble_sel <= `H;
        end
        else if (tpe_txcarrier | tpe_state[TPE_SEND_PREAMBLE]) begin
...
// Registered MII/GMII data signals
  always @(`TX_RST_MODE) begin
    if (~rst_clk_tx_n) begin
      ...
      else if(nibble_sel) begin
        gmii_txd_o[3:0] <= tx_data[3:0];
      end
      else begin
        gmii_txd_o[3:0] <= tx_data[7:4];
      end
    end  
  end

...
// Internal reg and wire declarations.
  ...
  reg  [1:0]   slot_delay;            // Mod-4 counter used in GMII mode.
...
// Parameter declarations
  ...
  parameter IFG_LMT_GMII = 5'b01100;
...
//---------------
  ...
  assign ifg_value = (sct_portselect) ? (IFG_LMT_MII - {1'b0,ifg_min,1'b0}) : 
                                        (IFG_LMT_GMII - {2'b0,ifg_min});
  assign ifg_clr = (sct_portselect) ? ~ifg_counter[4] : ~ifg_counter[3];
  assign ifg_cnt_lmt = (sct_portselect) ?  (ifg_counter == IFG_LMT_MII) : 
                                           (ifg_counter == IFG_LMT_GMII);
...
//------------
    ...
    // This slot_delay counter determines the slot time for GMII and MII
    // (4096 Bit times and 512 Bit times).
    always @(`TX_RST_MODE) begin
      if(~rst_clk_tx_n) begin
        slot_delay <= 2'b00;
      end
        ...
        case (sct_portselect)
          1'b1: slot_delay <= 2'b11;
          default: begin
            case (tpe_backoff_strt | strt_pause_timer)
              1'b1: slot_delay <= 2'b00;
              default: slot_delay <= slot_delay + 2'b01;
            endcase
          end
        endcase
      end
  ...
  assign stx_slottimetic = (&slot_timer) & (&slot_delay);

  assign pause_quanta_tic = (sct_portselect) ? stx_slottimetic :
                            (&slot_timer[3:0]) & (&slot_delay);

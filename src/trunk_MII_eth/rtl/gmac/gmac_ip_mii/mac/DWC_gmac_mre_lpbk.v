...
// local wire and register declarations.
  ...
  reg                    incr_wr_ptr_s_r;         // incr_wr_ptr_s registered 
                                                  // once
  reg                    incr_wr_ptr_s_2r;        // incr_wr_ptr_s registered 
                                                  // twice.

  reg  [DATA_WIDTH-1:0]  mem[8:0]; // Register memory.
...
// --------------------------------
  always @(`TX_RST_MODE) begin
    if (!rst_clk_tx_n) begin
      ...
      mem[5] <= {DATA_WIDTH{1'b0}};
      mem[6] <= {DATA_WIDTH{1'b0}};
      mem[7] <= {DATA_WIDTH{1'b0}};
      mem[8] <= {DATA_WIDTH{1'b0}};
    end
    else begin
      case (wptr)
        4'b0000: mem[0] <= wdata;
  ...
  assign rdata_int = (rptr == 4'b0000) ? mem[0] :
                     (rptr == 4'b0001) ? mem[1] :
                     (rptr == 4'b0010) ? mem[2] :
                     (rptr == 4'b0011) ? mem[3] :
                     (rptr == 4'b0100) ? mem[4] :
...
// --------------------------------
  ...
  assign rptr_nxt = (rptr == (scr_portselect ? 4'b1000 : 4'b0100)) ? 4'b0000 : rptr_pls1;
...
// ---------------------------------------
  ...
  assign wptr_nxt = (wptr == (sct_portselect ? 4'b1000 : 4'b0100)) ? 4'b0000 : wptr_pls1;
  ...
  always @(`RX_RST_MODE) begin
    if (!rst_clk_rx_n) begin
      ...
      incr_wr_ptr_s_r  <= incr_wr_ptr_s;
      incr_wr_ptr_s_2r <= incr_wr_ptr_s_r;
    end  
  end

  assign rd_offset = (scr_portselect) ? incr_wr_ptr_s_2r : incr_wr_ptr_s;
